...
export function BlogApproval() {
  ...
  const fetchBlogs = async () => {
    ...
    try {
      ...
      setReviewedBlogs(
        all.filter((b) => b.status === "APPROVED" || b.status === "REJECTED"),
      );
    } catch (err) {
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-[#0A1128]/70">
              Review and moderate user-submitted blogs
            </p>
          </div>
          <button
            onClick={fetchBlogs}
                  ...
                  <div className="space-y-4">
                    {pendingBlogs.map((blog) => (
                      ...
                      >
                        ...
                        <p className="text-[#0A1128]/75 text-sm mb-4 line-clamp-3">
                          {blog.excerpt}
                        </p>
                  ...
                  <div className="bg-white rounded-lg shadow-lg overflow-hidden">
                    <table className="w-full">
                      <thead className="bg-[#0A1128]/5">
                        <tr>
                          <th className="px-6 py-3 text-left text-xs font-medium text-[#0A1128]/70 uppercase tracking-wider">
                            Title
                          </th>
                          <th className="px-6 py-3 text-left text-xs font-medium text-[#0A1128]/70 uppercase tracking-wider">
                            Author
                      ...
                      <tbody className="divide-y divide-[#0A1128]/10">
                        {reviewedBlogs.map((blog) => (
                          <tr key={blog.id} className="hover:bg-[#0A1128]/5">
                            <td className="px-6 py-4 text-sm font-medium text-[#0A1128] max-w-[180px] truncate">
                              {blog.title}
                            </td>
                            <td className="px-6 py-4 text-sm text-[#0A1128]">
                              {blog.authorName}
                            </td>
                            <td className="px-6 py-4 text-sm text-[#0A1128]/70">
                              {blog.reviewedAt ?? "—"}
                            </td>
                            ...
                            <td className="px-6 py-4 text-sm text-[#0A1128]/70 max-w-[160px]">
                              {blog.rejectionReason ? (
                                <span
                                  className="line-clamp-2 text-red-600/80 italic"
                                  title={blog.rejectionReason}
                                >
                                  {blog.rejectionReason}
                                </span>
                              ) : (
              ...
              <div className="bg-white rounded-lg shadow-lg p-6 sticky top-4">
                {selectedBlog ? (
                  <div>
                    ...
                    <div className="space-y-4">
                      ...
                      {selectedBlog.category && (
                        <div>
                          <p className="text-xs text-[#0A1128]/60 mb-1">
                            Category
                          </p>
                      ...
                      <div>
                        ...
                        <p className="text-sm text-[#0A1128]/80 leading-relaxed">
                          {selectedBlog.excerpt}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-[#0A1128]/60 mb-2">
                          Featured Image
                        </p>
                      ...
                      {selectedBlog.status === "PENDING" && (
                        <div className="flex gap-2 pt-2">
                          ...
                          <button
                            onClick={() =>
                              handleReject(selectedBlog.id, selectedBlog.title)
                            }
                            disabled={actionLoading === selectedBlog.id}
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-[#0A1128]/50 text-sm">
