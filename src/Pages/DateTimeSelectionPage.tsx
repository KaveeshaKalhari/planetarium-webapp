...
export function DateTimeSelectionPage() {
  ...
  const [selectedTime, setSelectedTime] = useState<string>(
    incoming?.showDetails?.showTime || "",
  );
  const [selectedLanguage, setSelectedLanguage] = useState<string>(
    incoming?.showDetails?.language
      ? incoming.showDetails.language.toLowerCase()
      : "",
  );
  ...
  const [schoolForm, setSchoolForm] = useState<SchoolFormData>(
    incoming?.schoolInfo || emptySchoolForm,
  );
  ...
  const validateSchoolForm = (): boolean => {
    ...
    if (!schoolForm.schoolAddress.trim())
      errors.schoolAddress = "Address is required";
    if (!schoolForm.contactNumber.trim())
      errors.contactNumber = "Contact number is required";
    else if (!/^\+?[\d\s\-]{7,15}$/.test(schoolForm.contactNumber))
      errors.contactNumber = "Enter a valid phone number";
    if (!schoolForm.email.trim()) errors.email = "Email is required";
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(schoolForm.email))
      errors.email = "Enter a valid email address";
    if (!schoolForm.studentCount.trim())
      errors.studentCount = "Student count is required";
    else if (
      isNaN(Number(schoolForm.studentCount)) ||
      Number(schoolForm.studentCount) < 1
    )
      errors.studentCount = "Enter a valid number";
    if (!schoolForm.gradeLevel) errors.gradeLevel = "Grade level is required";
    if (!schoolForm.teacherName.trim())
      errors.teacherName = "Teacher / contact name is required";
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      const match = shows.find(
        (s) => s.showDate === localDate && s.showTime === selectedTime,
      );
      if (!match) {
        setNoShowError(true);
        return;
      ...
      if (getDateSessionType(selectedDate) === "SCHOOL") {
        ...
        navigate("/seat-selection", {
          state: { showId: match.id, showDetails: match },
        });
      }
    } catch {
      setNoShowError(true);
  ...
  const isSchoolSession = selectedDate
    ? getDateSessionType(selectedDate) === "SCHOOL"
    : false;
  const canContinue =
    selectedDate &&
    selectedTime &&
    (isWeekend(selectedDate) || selectedLanguage || isSchoolSession);
  ...
  const sf = (field: keyof SchoolFormData) => ({
    ...
    onChange: (
      ...
      if (schoolFormErrors[field])
        setSchoolFormErrors((prev) => ({ ...prev, [field]: "" }));
    },
  });
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-[#F0F6FA]">
      ...
      <div className="bg-gradient-to-r from-[#0A1128] via-[#001F54] to-[#034078] text-white relative overflow-hidden shrink-0">
        ...
        <div
          className={`max-w-7xl mx-auto px-6 py-3 relative z-10 transition-all duration-500 ${mounted ? "opacity-100" : "opacity-0"}`}
        >
      ...
      {/* ── Main Content ── */}
      <div
        className={`flex-1 overflow-hidden transition-all duration-500 ${mounted ? "opacity-100" : "opacity-0"}`}
      >
        <div className="h-full max-w-7xl mx-auto px-4 py-3 grid grid-cols-12 gap-4">
          {/* ── Calendar ── */}
          <div className="col-span-5 bg-white rounded-xl shadow-sm border border-[#0A1128]/8 p-4 flex flex-col overflow-y-auto">
            ...
            <div className="grid grid-cols-7 gap-0.5 mb-1">
              {dayNames.map((d) => (
                <div
                  key={d}
                  className="text-center text-[10px] font-bold text-[#0A1128]/40 py-1"
                >
                  {d}
                </div>
              ))}
            ...
            <div key={calendarKey} className="grid grid-cols-7 gap-0.5">
              {calendarDays.map((date, index) => {
                ...
                const type = getDateSessionType(date);
                const isSpecial =
                  type === "PUBLIC_TAMIL" || type === "PUBLIC_ENGLISH";
                ...
                return (
                  <button
                    ...
                    className={`aspect-square rounded-lg text-[11px] font-semibold transition-all duration-150 relative flex flex-col items-center justify-center
                                            ${
                                              isPast
                                                ? "text-[#0A1128]/15 cursor-not-allowed"
                                                : isSelected
                                                  ? "bg-[#1282A2] text-white shadow-md scale-105 z-10"
                                                  : isSchool
                                                    ? "bg-[#034078]/8 text-[#0A1128] hover:bg-[#034078]/20 border border-[#034078]/15"
                                                    : isSpecial
                                                      ? "bg-purple-100 text-[#0A1128] hover:bg-purple-200 border border-purple-300"
                  ...
                  >
                    ...
                    {!isPast && !isSelected && (
                      <span className="text-[7px] opacity-50 leading-none">
                        {getDayLabel(date)}
                      </span>
                    )}
                    {isToday && !isSelected && (
                      <div className="absolute bottom-0.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-[#1282A2]" />
                    )}
                  </button>
                );
              })}
            ...
            <div className="flex items-center justify-center gap-3 mt-2 pt-2 border-t border-[#0A1128]/8 flex-wrap">
              {[
                {
                  bg: "bg-[#034078]/15 border border-[#034078]/30",
                  label: "School",
                },
                {
                  bg: "bg-[#1282A2]/15 border border-[#1282A2]/30",
                  label: "Public",
                },
                {
                ...
                <div key={l.label} className="flex items-center gap-1">
                  ...
                  <span className="text-[10px] text-[#0A1128]/60">
                    {l.label}
                  </span>
                </div>
              ))}
            ...
            {/* Session-type reference cards, moved below the calendar legend */}
            <div className="grid grid-cols-2 gap-2 mt-3 pt-3 border-t border-[#0A1128]/8">
              {[
                {
                  label: "School",
                  sub: "Mon–Fri",
                  bg: "bg-[#034078]/8",
                  border: "border-[#034078]",
                },
                {
                  label: "Public",
                  sub: "Weekends",
                ...
                <div
                  key={s.label}
                  className={`${s.bg} border-l-2 ${s.border} p-2 rounded-lg flex items-center gap-2`}
                >
                  <div>
                    <p className="font-bold text-[#0A1128] text-[11px]">
                      {s.label}
                    </p>
          ...
          {/* ── Show Time + Language + Summary + CTA (single stacked column) ── */}
          <div className="col-span-7 flex flex-col gap-3 overflow-y-auto">
            <div className="bg-white rounded-xl shadow-sm border border-[#0A1128]/8 p-3">
              <div className="flex items-center gap-1.5 mb-2">
                <Clock className="w-3.5 h-3.5 text-[#1282A2]" />
                <h3 className="font-bold text-[#0A1128] text-sm">Show Time</h3>
                <span className="text-[10px] text-[#0A1128]/40 ml-auto">
                  45 min
                </span>
              </div>
              <div className="space-y-2">
                {showTimes.map((show) => {
                  const sessionType = selectedDate
                    ? getDateSessionType(selectedDate)
                    : "";
                  const isLockedOut =
                    (sessionType === "PUBLIC_TAMIL" &&
                      show.id === "afternoon") ||
                    (sessionType === "PUBLIC_ENGLISH" && show.id === "morning");
                  ...
                  return (
                    <button
                      ...
                      className={`w-full p-2.5 rounded-lg border-2 transition-all duration-150 text-left flex items-center justify-between
                                                ${
                                                  !selectedDate || isLockedOut
                                                    ? "border-[#0A1128]/8 bg-[#0A1128]/4 text-[#0A1128]/30 cursor-not-allowed"
                                                    : isActive
                                                      ? "border-[#1282A2] bg-[#1282A2]/8 shadow-sm"
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <div>
                          <p
                            className={`font-bold text-xs ${!selectedDate || isLockedOut ? "text-[#0A1128]/30" : "text-[#0A1128]"}`}
                          >
                            {show.time}
                          </p>
                          <p
                            className={`text-[10px] ${!selectedDate || isLockedOut ? "text-[#0A1128]/20" : "text-[#0A1128]/60"}`}
                          >
                            {show.label}
                          </p>
                        </div>
                      ...
                      {isActive && (
                        <div className="w-4 h-4 rounded-full bg-[#1282A2] flex items-center justify-center">
                          <svg
            ...
            {selectedDate && isWeekday(selectedDate) && !isSchoolSession && (
              <div className="bg-white rounded-xl shadow-sm border border-[#0A1128]/8 p-3 fade-up">
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span className="text-[10px] px-1.5 py-0.5 bg-[#034078]/10 text-[#034078] rounded-full ml-auto">
                    Weekday
                  </span>
                </div>
                      ...
                      className={`w-full p-2 rounded-lg border-2 transition-all duration-150 text-left flex items-center justify-between
                                                ${
                                                  selectedLanguage === lang.id
                                                    ? "border-[#034078] bg-[#034078]/8"
                                                    : "border-[#0A1128]/10 hover:border-[#034078]/40 hover:bg-[#034078]/4"
                                                }`}
                    >
                      <div className="flex items-center gap-2">
                        <span className="text-sm">{lang.icon}</span>
                        <span className="font-medium text-[#0A1128] text-xs">
                          {lang.name}
                        </span>
                      </div>
                      {selectedLanguage === lang.id && (
                        <div className="w-3.5 h-3.5 rounded-full bg-[#034078] flex items-center justify-center">
                          <svg
            ...
            {isSchoolSession && (
              <div className="bg-gradient-to-br from-[#034078]/10 to-[#034078]/5 border border-[#034078]/30 rounded-xl p-3 fade-up flex items-start gap-2">
                ...
                <div>
                  <p className="font-bold text-[#034078] text-xs">
                    School Program Selected
                  </p>
                  <p className="text-[10px] text-[#034078]/70 mt-0.5">
                    You'll fill in school details before proceeding to seat
                    selection.
                  </p>
                </div>
              </div>
            ...
            {/* ── Summary + CTA, stacked below Show Time/Language ── */}
              ...
              <div className="border border-purple-300 bg-gradient-to-r from-purple-50 to-indigo-50 rounded-xl p-3 flex items-start gap-2 fade-up">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-purple-700 mt-0.5">
                    {getDateSessionType(selectedDate) === "PUBLIC_TAMIL"
                      ? "🌐 Tamil Medium — Morning (10:00 AM)"
                      : "🌐 English Medium — Afternoon (3:00 PM)"}
                  </p>
                </div>
              </div>
            )}
            ...
            {selectedDate ? (
              <div className="bg-gradient-to-br from-[#0A1128] to-[#001F54] rounded-xl shadow-lg p-4 text-white">
                <h3 className="font-bold mb-3 text-[10px] tracking-widest uppercase text-white/50">
                  Your Selection
                </h3>
                <div className="space-y-2">
                  {[
                      ...
                      ? [
                          {
                            label: "Time",
                            value:
                              showTimes.find((s) => s.id === selectedTime)
                                ?.time || "",
                          },
                        ]
                      : []),
                    ...
                    <div
                      key={row.label}
                      className={`flex justify-between items-start py-2 ${i < arr.length - 1 ? "border-b border-white/10" : ""}`}
                    >
                      <span className="text-white/50 text-xs">{row.label}</span>
                      <span className="font-semibold text-xs text-right max-w-[55%]">
                        {row.value}
                      </span>
                    </div>
                  ))}
                ...
                {isSchoolSession && (
                  <div className="mt-3 pt-3 border-t border-white/10">
                    <div className="flex items-center gap-1.5 text-[#219EBC] text-[10px]">
                      ...
                      <span>
                        School registration form required before seat selection
                      </span>
                    </div>
                  </div>
                )}
              ...
              <div className="bg-[#0A1128]/5 rounded-xl border-2 border-dashed border-[#0A1128]/15 py-10 flex items-center justify-center">
                <div className="text-center">
                  <CalendarIcon className="w-8 h-8 text-[#0A1128]/20 mx-auto mb-2" />
                  <p className="text-xs text-[#0A1128]/35">
                    Select a date to see your booking summary
                  </p>
                </div>
              </div>
            )}
            ...
            {selectedDate && isWeekday(selectedDate) && (
              <div className="rounded-xl p-3 text-white bg-gradient-to-br from-[#034078] to-[#001F54] border border-[#1282A2]/20 shadow-md fade-up">
                <div className="flex items-start gap-2 mb-2">
                  <div className="bg-[#1282A2] p-1.5 rounded-lg shrink-0">
                    <MessageSquare className="w-3.5 h-3.5 text-white" />
                  </div>
                  <div>
                    <h3 className="font-bold text-white text-xs">
                      Need a Custom School Session?
                    </h3>
                    <p className="text-[10px] text-white/60">
                      Tailor content for your grade level
                    </p>
                  </div>
                </div>
                <button
                ...
                >
                  <MessageSquare className="w-3 h-3" /> Chat with Our Team{" "}
            ...
            <div className="bg-[#1282A2]/8 border border-[#1282A2]/20 rounded-xl p-3">
              <div className="flex items-start gap-2">
                ...
                <ul className="text-[10px] text-[#0A1128]/60 space-y-1">
                  ...
                  <li>
                    Special Tamil &amp; English sessions on 2nd Sat &amp; 4th
                    Sun
                  </li>
                </ul>
              </div>
            ...
            <button
              ...
              disabled={!canContinue || loadingShows}
            ...
            {noShowError && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-2 text-xs text-red-600 text-center">
                No show available for this slot. Please try another.
              </div>
            )}
            {!canContinue && !noShowError && (
              <p className="text-[10px] text-center text-[#0A1128]/40">
                {!selectedDate
                  ? "Select a date"
                  : !selectedTime
                    ? "Select a show time"
      ...
      {showSchoolForm && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setShowSchoolForm(false);
          }}
        >
          <div className="modal-in bg-white rounded-2xl shadow-2xl w-full max-w-xl flex flex-col max-h-[90vh]">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-white/60 text-xs">
                    Please fill in your school details to continue
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="overflow-y-auto p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] font-semibold text-[#0A1128]/70 uppercase tracking-wide flex items-center gap-1 mb-1">
                    <GraduationCap className="w-3 h-3 text-[#1282A2]" /> School
                ...
                <div>
                  <label className="text-[11px] font-semibold text-[#0A1128]/70 uppercase tracking-wide flex items-center gap-1 mb-1">
                    <Phone className="w-3 h-3 text-[#1282A2]" /> Contact Number{" "}
                  ...
                  />
                  {schoolFormErrors.contactNumber && (
                    <p className="text-[10px] text-red-500 mt-1 flex items-center gap-1">
                      <AlertCircle className="w-2.5 h-2.5" />
                      {schoolFormErrors.contactNumber}
                    </p>
              ...
              <div>
                <label className="text-[11px] font-semibold text-[#0A1128]/70 uppercase tracking-wide flex items-center gap-1 mb-1">
                  <MapPin className="w-3 h-3 text-[#1282A2]" /> School Address{" "}
                  <span className="text-red-400">*</span>
                </label>
                <input
                  type="text"
                ...
                />
                {schoolFormErrors.schoolAddress && (
                  <p className="text-[10px] text-red-500 mt-1 flex items-center gap-1">
                    <AlertCircle className="w-2.5 h-2.5" />
                    {schoolFormErrors.schoolAddress}
                  </p>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] font-semibold text-[#0A1128]/70 uppercase tracking-wide flex items-center gap-1 mb-1">
                    <Mail className="w-3 h-3 text-[#1282A2]" /> Email{" "}
                    <span className="text-red-400">*</span>
                  </label>
                  <input
                    type="email"
                ...
                <div>
                  <label className="text-[11px] font-semibold text-[#0A1128]/70 uppercase tracking-wide flex items-center gap-1 mb-1">
                    <Users className="w-3 h-3 text-[#1282A2]" /> Teacher /
                    In-charge <span className="text-red-400">*</span>
                  </label>
                  <input
                    type="text"
                    placeholder="e.g. Mrs. Perera"
                    {...sf("teacherName")}
                    className={`w-full px-3 py-2 text-sm rounded-lg border-2 outline-none transition-colors bg-[#F8FAFC] placeholder:text-[#0A1128]/30
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] font-semibold text-[#0A1128]/70 uppercase tracking-wide flex items-center gap-1 mb-1">
                    <Users className="w-3 h-3 text-[#1282A2]" /> Student Count{" "}
                    <span className="text-red-400">*</span>
                  </label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="text-[11px] font-semibold text-[#0A1128]/70 uppercase tracking-wide flex items-center gap-1 mb-1">
                    <GraduationCap className="w-3 h-3 text-[#1282A2]" /> Grade
                    Level <span className="text-red-400">*</span>
                  </label>
                  <select
                    value={schoolForm.gradeLevel}
                    onChange={(e) => {
                      setSchoolForm((prev) => ({
                        ...prev,
                        gradeLevel: e.target.value,
                      }));
                      if (schoolFormErrors.gradeLevel)
              ...
              <div>
                <label className="text-[11px] font-semibold text-[#0A1128]/70 uppercase tracking-wide flex items-center gap-1 mb-1">
                  <FileText className="w-3 h-3 text-[#1282A2]" /> Other
                  Information{" "}
                  <span className="text-[#0A1128]/35 font-normal normal-case tracking-normal">
                    (optional)
                  </span>
                </label>
                <textarea
                  placeholder="Special requirements, accessibility needs, curriculum topic focus, etc."
                  rows={3}
              ...
              <div className="bg-[#EFF6FF] border border-[#BAE6FD] rounded-lg p-3 flex items-start gap-2">
                ...
                <p className="text-[10px] text-[#0A1128]/70">
                  School details help us tailor the show experience for your
                  students. This information will be shared with our education
                  team. <span className="text-red-400">*</span> Required fields.
                </p>
              </div>
            </div>
            ...
            <div className="px-5 py-4 border-t border-[#E2E8F0] flex gap-3 shrink-0 rounded-b-2xl bg-[#F8FAFC]">
              ...
              >
                <GraduationCap className="w-4 h-4" /> Proceed to Seat Selection{" "}
